...
public:
    ...
    bool isDuplicate(const std::string& msg, int timestamp) {
        while (!msgQueue.empty() && timestamp - msgQueue.front().second > window) {
            lastSeen.erase(msgQueue.front().first);
            msgQueue.pop();
        }
        if (lastSeen.count(msg)) return true;
        lastSeen[msg] = timestamp;
        msgQueue.push({msg, timestamp});
        return false;
    }
