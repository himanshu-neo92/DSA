// Problem: All Air Paths (DFS)
// Write a function to record travel from source to destination.
// Given source and destination params, print all paths from src to dest.
//
// ==================== PERFORMANCE ====================
// Time Complexity:  O(V!) worst case - explores all permutations of vertices
// Space Complexity: O(V+E) - adjacency list + recursion stack + visited set
//
// Key concepts:
// - DFS backtracking to enumerate all paths
  ...
  void dfs(const std::string& current, const std::string& dest,
    ...
    for (const auto& neighbor : adjList[current]) {
      if (visited.find(neighbor) == visited.end()) {
        ...
        dfs(neighbor, dest, path, visited, allPaths);
        path.pop_back();
...
public:
  ...
  std::vector<std::vector<std::string>> getAllPaths(const std::string& src, const std::string& dest) {
    ...
    dfs(src, dest, path, visited, allPaths);
    return allPaths;
  }
