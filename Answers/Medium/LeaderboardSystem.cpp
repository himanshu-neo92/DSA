...
public:
    void addScore(int userId, int score) {
        if (userScores.count(userId)) {
            scoreSet.erase(scoreSet.find({userScores[userId], userId}));
        }
        userScores[userId] += score;
        scoreSet.insert({userScores[userId], userId});
    }

    std::vector<int> top(int k) {
        std::vector<int> result;
        for (auto it = scoreSet.rbegin(); it != scoreSet.rend() && k > 0; ++it, --k) {
